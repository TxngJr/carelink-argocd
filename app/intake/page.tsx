...
export default function IntakePage() {
  ...
  async function save(event: React.FormEvent) {
    ...
    if (!encounterId) {
      setMessageTone('danger')
      setMessage('กรุณาเลือกผู้ป่วยจากคิว MHT')
      return
    }
    setBusy(true)
    setMessage('')
    try {
      ...
      setMessage('บันทึกการซักประวัติครบแล้ว พร้อมส่งต่อแพทย์')
    } catch (cause) {
      setMessageTone('danger')
    ...
    <StaffShell role="nurse" displayName="พยาบาลจุดซักประวัติ">
      <div className={styles.page}>
        <header className={styles.hero}>
          <div className={styles.heroIcon}><ClipboardCheck size={25} /></div>
          <div className={styles.heroCopy}>
            <span className="eyebrow">NURSE INTAKE & TRIAGE · MHT</span>
            <h1>ซักประวัติผู้ป่วย</h1>
            <p>ตรวจข้อมูล Pre-visit, Vital signs และบันทึกข้อมูลสำคัญก่อนส่งต่อแพทย์</p>
          </div>
          <div className={`${styles.patientStatus} ${encounterId ? styles.patientSelected : ''}`}>
            {loadingContext ? <LoaderCircle size={17} className={styles.spin} /> : encounterId ? <CheckCircle2 size={17} /> : <UserRound size={17} />}
            <div><span>{encounterId ? 'กำลังซักประวัติ' : 'รอเลือกผู้ป่วย'}</span><strong>{patient?.display_name || 'เลือกจากคิว MHT ด้านล่าง'}</strong></div>
          </div>
        </header>

        {message && <div className={`inline-alert ${messageTone} ${styles.alert}`}>{message}</div>}

        {!context && !loadingContext && (
          <section className={styles.startCard}>
            <span className={styles.startIcon}><Sparkles size={28} /></span>
            <div><h2>เริ่มจากเลือกผู้ป่วยในคิว MHT</h2><p>กดเลือก Encounter จากคิวด้านล่าง ระบบจะดึงข้อมูลผู้ป่วย, Pre-visit และ Vital signs ขึ้นมาให้อัตโนมัติ</p></div>
            <ArrowRight size={22} />
          </section>
        )}
        ...
        {context && (
          <section className={styles.snapshotGrid}>
            <article className={styles.patientSnapshot}>
              <div className={styles.cardHeading}><span><UserRound size={19} /></span><div><h2>Patient snapshot</h2><p>ข้อมูลที่ควรเห็นก่อนเริ่มซักประวัติ</p></div></div>
            ...
            <article className={styles.vitalsCard}>
              <div className={styles.cardHeading}><span className={styles.vitalsIcon}><HeartPulse size={19} /></span><div><h2>Vital signs ล่าสุด</h2><p>ใช้ประกอบการประเมินก่อนพบแพทย์</p></div></div>
              {vitals ? (
        ...
        <form className={styles.formCard} onSubmit={save}>
          <div className={styles.formHeader}>
            <div className={styles.cardHeading}><span><Stethoscope size={20} /></span><div><h2>Clinical nursing workup</h2><p>แบ่งข้อมูลเป็นหมวดเพื่ออ่านและกรอกได้เร็วขึ้น</p></div></div>
            <span className={styles.requiredNote}>* จำเป็น</span>
          </div>

          <section className={styles.formSection}>
            <div className={styles.sectionLabel}><span><MessageSquareText size={18} /></span><div><h3>1. อาการปัจจุบัน</h3><p>สรุปเหตุผลที่มารับบริการและลำดับอาการ</p></div></div>
          ...
          <section className={styles.formSection}>
            <div className={styles.sectionLabel}><span className={styles.pillIcon}><Pill size={18} /></span><div><h3>2. ยา โรคประจำตัว และประวัติสำคัญ</h3><p>ทบทวนข้อมูลที่มีผลต่อการรักษา</p></div></div>
          ...
          <section className={styles.formSection}>
            <div className={styles.sectionLabel}><span className={styles.treatmentIcon}><History size={18} /></span><div><h3>3. ประวัติการรักษา</h3><p>ช่วยให้แพทย์เห็น treatment timeline ได้เร็ว</p></div></div>
            <div className={styles.twoCol}>
          ...
          <section className={styles.formSection}>
            <div className={styles.sectionLabel}><span className={styles.activityIcon}><Activity size={18} /></span><div><h3>4. อาการร่วมและพฤติกรรมสุขภาพ</h3><p>ข้อมูลประกอบการประเมินภาพรวม</p></div></div>
            <div className={styles.twoCol}>
          ...
          <section className={`${styles.formSection} ${styles.safetySection}`}>
            <div className={styles.sectionLabel}><span className={styles.safetyIcon}><ShieldCheck size={18} /></span><div><h3>5. Safety check & Triage</h3><p>ยืนยันข้อมูลสำคัญและกำหนดระดับเร่งด่วนก่อนส่งต่อ</p></div></div>
            <label className={styles.confirmCheck}><input type="checkbox" checked={Boolean(extra.allergies_confirmed)} onChange={(e) => updateExtra('allergies_confirmed', e.target.checked)} /><span><strong>ตรวจทานข้อมูลแพ้ยากับผู้ป่วยแล้ว</strong><small>ควรยืนยันกับผู้ป่วยทุกครั้งก่อนส่งต่อ</small></span></label>
            <div className={styles.twoCol}>
            ...
            <div className={styles.priorityChoices}>
              <label className={urgent ? styles.priorityActive : ''}><input type="checkbox" checked={urgent} onChange={(e) => setUrgent(e.target.checked)} /><AlertTriangle size={17} /><span>Urgent</span></label>
              <label className={fastTrack ? styles.fastTrackActive : ''}><input type="checkbox" checked={fastTrack} onChange={(e) => setFastTrack(e.target.checked)} /><Syringe size={17} /><span>Fast-track</span></label>
            </div>
            <div className={styles.demoNotice}><AlertTriangle size={17} /><span>Triage และ functional-status เป็นข้อมูลจำลองเพื่อการศึกษา ไม่ใช่ clinical protocol จริง</span></div>
          </section>

          <div className={styles.actionBar}>
            <div><ClipboardCheck size={18} /><span><strong>{patient?.display_name || 'ยังไม่ได้เลือกผู้ป่วย'}</strong><small>{encounterId ? 'ตรวจข้อมูลให้ครบก่อนบันทึกและส่งต่อ' : 'เลือกผู้ป่วยจากคิว MHT ก่อน'}</small></span></div>
            <button className={`button primary large ${styles.submitButton}`} disabled={busy || !encounterId}>
              {busy ? <><LoaderCircle size={18} className={styles.spin} /> กำลังบันทึก…</> : <><CheckCircle2 size={18} /> บันทึกและส่งต่อแพทย์ <ArrowRight size={18} /></>}
            </button>
          </div>
        </form>

        <section className={styles.queueSection}>
          <div className={styles.queueHeading}><span><UserRound size={20} /></span><div><h2>คิวรอซักประวัติ MHT</h2><p>เลือกผู้ป่วยเพื่อเปิด Clinical nursing workup ด้านบน</p></div></div>
