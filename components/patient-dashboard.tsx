  ...
  return (
    <section className="journey-card">
      ...
      <div className="journey-metrics">
        <div><span>คิวข้างหน้า</span><strong>{journey.queue_ahead ?? 0}</strong><small>คิว</small></div>
        <div>
          <span>เวลารอโดยประมาณ</span>
          <strong>{journey.est_wait_min ?? 0}{journey.est_wait_band ? `–${(journey.est_wait_min || 0) + journey.est_wait_band}` : ''}</strong>
          <small>นาที</small>
        </div>
        <div><span>จุดถัดไป</span><strong className="station-short">{journey.next_station || 'END'}</strong><small>{journey.next_station_name || 'สิ้นสุดการรักษา'}</small></div>
      </div>

      <div className="journey-progress">
        <div className="progress-label"><span>ความคืบหน้าการรับบริการวันนี้</span><strong>{journey.step_current || 0}/{journey.step_total || 0} ขั้นตอน</strong></div>
      ...
      <div className="route-timeline">
        {(journey.route || []).map((step, index) => (
          <div className={`route-step ${step.status}`} key={`${step.station_code}-${index}`}>
            ...
            <div><strong>{step.station_code}</strong><span>{stationMap.get(step.station_code)?.name || step.station_code}</span></div>
...
function PrevisitTab({ onSaved }: { onSaved: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    clientApi.getPrevisit().then((res) => {
      ...
      setMessage(cause instanceof Error ? cause.message : 'โหลดข้อมูลก่อนมารับบริการไม่สำเร็จ')
    })
    return () => { active = false }
  }, [])
  ...
  return (
    <div className="patient-section">
      ...
      <form className="patient-card form-card" onSubmit={handleSavePrevisit}>
        ...
        <label><span>ประวัติแพ้ยา / แพ้อาหาร (ถ้ามี)</span><input value={allergies} onChange={(e) => setAllergies(e.target.value)} placeholder="เช่น Penicillin, อาหารทะเล หรือ ไม่มี" /></label>

        <div className="form-two">
          <label><span>ยาที่รับประทานประจำ</span><input value={meds} onChange={(e) => setMeds(e.target.value)} placeholder="ชื่อยาและความถี่" /></label>
          <label><span>ยาสมุนไพร / อาหารเสริม</span><input value={herbals} onChange={(e) => setHerbals(e.target.value)} placeholder="ชื่อสมุนไพร หรือ วิตามิน" /></label>
        </div>

        <div className="form-two">
          <label><span>สิทธิการรักษา</span><select value={payer} onChange={(e) => setPayer(e.target.value)}><option value="UC (บัตรทอง)">UC (บัตรทอง / 30 บาท)</option><option value="SSS (ประกันสังคม)">SSS (ประกันสังคม)</option><option value="CSMBS (ข้าราชการ)">CSMBS (สวัสดิการข้าราชการ)</option><option value="Self-Pay (ชำระเงินเอง)">ชำระเงินเอง</option></select></label>
          <label><span>เบอร์ติดต่อฉุกเฉิน</span><input value={contact} onChange={(e) => setContact(e.target.value)} placeholder="0812345678" /></label>
        </div>
...
function HelpAndProfileTab({ user, displayName, onLogout }: { user: PublicUser | null; displayName: string; onLogout: () => Promise<void> }) {
  ...
  const [helpTone, setHelpTone] = useState<MessageTone>('info')

  async function handleSendHelp(e: FormEvent) {
    e.preventDefault()
    if (!helpMsg.trim()) return
    setBusy(true); setHelpFeedback('')
    try {
      await clientApi.createHelpRequest(category, helpMsg.trim())
      setHelpTone('success')
      setHelpFeedback('ส่งคำขอความช่วยเหลือแล้ว ทีมเจ้าหน้าที่จะเห็นคำขอในระบบ')
      setHelpMsg('')
    } catch (cause) {
      setHelpTone('danger')
      setHelpFeedback(cause instanceof Error ? cause.message : 'ส่งคำขอความช่วยเหลือไม่สำเร็จ')
    } finally { setBusy(false) }
  }

  return (
    <div className="patient-section">
      <div className="profile-hero"><div className="avatar profile-avatar">{(user?.display_name || displayName).slice(0, 1)}</div><h1>{user?.display_name || displayName}</h1><span>บัญชีผู้ป่วย CareLink DynaFlow</span></div>

      <section className="patient-card">
        <div className="patient-card-head"><div><span className="eyebrow">HELP & ASSISTANCE</span><h2>ขอความช่วยเหลือระหว่างรับบริการ</h2></div><HelpCircle size={22} color="var(--brand)" /></div>
        <form onSubmit={handleSendHelp} style={{ display: 'grid', gap: 12 }}>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[{ id: 'directions', label: 'ถามเส้นทาง' }, { id: 'queue', label: 'สอบถามคิว' }, { id: 'clinical', label: 'อาการเปลี่ยนแปลง' }, { id: 'other', label: 'อื่น ๆ' }].map((c) => (
              <button key={c.id} type="button" className={`button ${category === c.id ? 'primary' : 'ghost'}`} style={{ minHeight: 34, padding: '0 12px', fontSize: '.8rem' }} onClick={() => setCategory(c.id as HelpRequestSubmission['category'])}>{c.label}</button>
            ))}
          </div>
          <textarea rows={3} value={helpMsg} onChange={(e) => setHelpMsg(e.target.value)} placeholder="บอกจุดที่คุณอยู่ หรือสิ่งที่ต้องการความช่วยเหลือ..." />
          {helpFeedback && <div className={`inline-alert ${helpTone}`}>{helpFeedback}</div>}
          <button className="button warning large full" disabled={busy || !helpMsg.trim()}>{busy ? 'กำลังส่งคำขอ…' : 'ส่งคำขอความช่วยเหลือ'}</button>
        </form>
      </section>
      ...
      <button className="button danger-outline large full" onClick={() => void onLogout()}><LogOut size={16} /> ออกจากระบบ</button>
...
export function PatientDashboard({ displayName }: { displayName: string }) {
  ...
  const refresh = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      setAppointment(nextAppointment); setJourney(nextJourney); setNotices(nextNotices || []); setUser(nextUser); setLoadError('')
    } catch (cause) {
      setLoadError(cause instanceof Error ? cause.message : 'โหลดข้อมูลผู้ป่วยไม่สำเร็จ')
  ...
  useEffect(() => {
    ...
    function fetchPatientData() {
      Promise.all([clientApi.getCurrentAppointment(), clientApi.getJourney(), clientApi.getNotifications(), clientApi.getPatientMe()])
        .then(([nextAppointment, nextJourney, nextNotices, nextUser]) => {
          if (!active) return
          setAppointment(nextAppointment); setJourney(nextJourney); setNotices(nextNotices || []); setUser(nextUser); setLoadError(''); setLoading(false)
        })
        .catch((cause) => {
          if (!active) return
          setLoadError(cause instanceof Error ? cause.message : 'เชื่อมต่อข้อมูลผู้ป่วยไม่สำเร็จ')
          setLoading(false)
        })
    }
    fetchPatientData()
    ...
    return () => { active = false; clearInterval(timer); es.close() }
  }, [])
  ...
  async function logout() {
    ...
    router.replace('/login/patient'); router.refresh()
  }
  ...
  async function handleReadNotice(id: string) {
    ...
    } catch (cause) { setLoadError(cause instanceof Error ? cause.message : 'อัปเดตการแจ้งเตือนไม่สำเร็จ') }
  }
  ...
  return (
    <div className="patient-shell">
      <header className="patient-header">
        <div className="patient-brand"><Image src="/logo-mark.svg" width={38} height={38} alt="" /><div><strong>CareLink</strong><span>Patient Journey 2.0</span></div></div>
        <div className="patient-header-actions">
          <button className="notification-button" onClick={() => setSection('notifications')} aria-label="การแจ้งเตือน"><Bell size={18} />{unread > 0 && <em>{unread}</em>}</button>
          <div className="avatar small patient-avatar">{(user?.display_name || displayName).slice(0, 1)}</div>
        </div>
      </header>

      <main className="patient-main">
        {loadError && <div className="inline-alert danger" role="alert" style={{ marginBottom: 14 }}>{loadError} <button className="button ghost" onClick={() => void refresh(true)}>ลองใหม่</button></div>}
        {loading ? (
          <div className="patient-loading"><Image src="/logo-mark.svg" width={54} height={54} alt="" /><span>กำลังโหลดข้อมูลการรับบริการของคุณ…</span></div>
        ) : (
          <>
            {section === 'home' && <>
            ...
            {section === 'profile' && <HelpAndProfileTab user={user} displayName={displayName} onLogout={logout} />}
          </>
        )}
      </main>
